...
<img alt="feedback.jpg" height="500" src="img%2Ffeedback.jpg" width="400"/>
...
в `/admin` есть функция просмотра базы данных

## Безопасность
1. Выдача токенов доступа:
   - Администраторы могут выдавать токены доступа сотрудникам и другим администраторам.
   - Каждый токен является уникальным.
2. Использование токенов:
   - Для выполнения команд и функций, требующих прав администратора или сотрудника, бот проверяет переданный токен.
   - Если токен действителен и соответствует роли пользователя, команда выполняется
